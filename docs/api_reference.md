...
- An **untouched stock file** for which the target ships a same-named file: swapped for the target's copy, listed in `auto_accepted`, absent from `stripped`. The operator did not write that file, so nothing of theirs is lost.
- An untouched stock file with **no counterpart on the target**, which the preset did not have enabled: dropped silently. It came from the catalog seed, not from anything the operator did.

"Untouched" is decided by content against **two** allow-lists, either of which is sufficient: the source runtime's default builtin preset, and the source runtime's baseline manifest (`ql-assets/data/<runtime>-plugins/manifest.json`). The manifest is broader — 63 files against the default preset's 53 — and for a handful of files records *different* content, because QLSM's own minqlx default preset ships a customised `motd.py` while the manifest records upstream's. Checking only one list mislabels files in both directions: stock plugins the default preset does not carry (`reset_acc.py`, `suppress_join_msg.py`) get reported as the operator's own work, and a preset holding upstream's untouched `motd.py` gets reported as having modified it. The manifest is keyed by bare filename and is therefore consulted for root-level paths only — `extras/motd.py` never borrows `motd.py`'s hash.
