...
**Plugin baseline:** `ql-assets/data/<runtime>-plugins/` (`minqlx-plugins` or `minqlxtended-plugins`) holds every plugin that ships with a host of that runtime, independent of any preset. Two delivery paths cover it, and they run in this order: `setup_host.yml` mirrors the directory onto the host at setup time (`{{ common_assets_dir }}/{{ runtime_plugins_dirname }}/`, `delete: yes`, via the shared `tasks/sync_common_plugins.yml`, which **Check for Updates** can also re-run on demand; see below); `add_qlds_instance.yml` then rsyncs the preset's `scripts/` into the new instance's plugin directory (also `delete: yes` — an operator's selection is authoritative for what it lists), and only after that backfills anything from the host baseline the preset didn't already provide (`--ignore-existing`). The baseline is therefore always present on every instance but never overrides a preset choice. The config editor's Plugins tab lists its root `.py` files (excluding `__init__.py` and `SYSTEM_PLUGINS`) next to the draft's own files as `shared` rows (`ui/plugin_pool.py`, appended in `draft_routes.get_draft_tree` for `draft_effective_runtime`), so an operator can enable a baseline plugin the preset doesn't carry. Enabling one only adds its name to `qlx_plugins`, and the backfill delivers the file. Editing one writes a local copy into the draft (plain `PUT content`), which then wins on deploy.

**Check for Updates (plugin pool refresh):** a host action that brings both delivery paths back in line with `ql-assets` without re-running host setup. The pool is resolved by name through `runtime_paths(host_runtime(host))['asset_plugins_dir']`, never by a `runtime == ...` branch, so any runtime that shares a pool resolves correctly.
- **Check** (`GET /api/hosts/<id>/plugin-updates`, `ui/task_logic/plugin_update_check.py`) is read-only and runs inside the request. `ui/update_checks.py` hashes the top-level `.py`/`.ql-plugin.json` files of the pool locally, then diffs them against:
  - the host's common pool, read by one ad-hoc `find … -exec sha256sum` through `run_host_ansible_adhoc()` in `ansible_runner.py` (15s SSH connect timeout under a 30s process timeout, so a hung host can't hold a web thread);
  - each instance's `configs/<host>/<id>/scripts/`, read locally.
- **Apply** (`POST …/plugin-updates/apply`, `ui/task_logic/ansible_plugin_update.py`, RQ task `apply_plugin_updates_task`, host lock):
  1. If selected, runs `update_common_plugins.yml`. It imports the same `tasks/sync_common_plugins.yml` as `setup_host.yml` with `skip_pip_install: true`, so the button path doesn't reinstall pip floors. A failure sets the host to `ERROR` and stops.
  2. Copies the selected pool files into each instance's `scripts/`.
  3. Queues `restart_instance` for the requested, non-stopped instances.

  Nothing reaches a running instance until it restarts. `sync_instance_configs_and_restart.yml` mirrors `scripts/` into the instance plugin directory with `delete: yes`, then backfills the host pool with `--ignore-existing`, so a refreshed pool file replaces the instance's old copy at that point. A newly copied file is still not loaded unless it is in the instance's `qlx_plugins` selection.
...
**Operator Model:** Directory of named operators (`name`, `steam_id64` unique, `default_level` 0-5) assignable as Owner (`qlx_owner` in `server.cfg`) or Admin from the Owner & Admins tab (instance editors) or panel (preset pages). Managed via `operator_routes.py` at `/api/operators`. Admin levels live only in the instance's minqlx Redis permission keys; QLSM keeps no admin table. `ui/task_logic/permission_read.py` reads them for the tab's GET endpoint, together with each admin's last in-game name (`minqlx:players:<id>:current_name`, else the name-history list) for rows not in the directory, and `ui/task_logic/access_permission_sync.py` writes only the changes from a config save (and the Add Instance list once after deploy). `access.txt` only carries Quake Live's own `admin`/`mod`/`ban` role lines; `ui/admin_permissions.py` strips any leftover numeric `steamid|level` line on save.
