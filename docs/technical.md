...
-   **minqlxtended hosts — display:** `lan_rate_always_on()` (`ui/lan_rate_policy.py`) reports the toggle as unconditionally on, since minqlxtended hooks `Sys_IsLANAddress` itself and excludes `force_rate.so` (see **Server Runtimes** above) — so neither the hook path nor the legacy NAT path can *change anything* there. The code paths themselves still run: `PUT /api/instances/<id>/lan-rate` is deliberately not gated on `lan_rate_always_on()` (see the stored-value bullet below), so a `true` write on a minqlxtended instance enqueues `reconfigure_instance_lan_rate`, which sees `Host.lan_rate_uses_hook` (set for every host at setup) and delegates to `apply_instance_hooks_logic(..., restart_service=True)` — **restarting the game server** to apply a setting the runtime already provides. QLSM's own forms render the toggle on and disabled and never let an operator switch it on themselves, so only a direct API call reaches that pointless restart. `frontend-react/src/utils/lanRateCompatibility.js` mirrors this display policy for the frontend — the two files are deliberate mirrors of each other; see [LAN Rate Policy Mirrors](development_guidelines.md#lan-rate-policy-mirrors) in Development Guidelines.
-   **minqlxtended hosts — stored value:** `host_supports_lan_rate()` classifies an always-on host as "fixed on," not "unsupported," so it returns `True` for minqlxtended by design; `would_enable_unsupported_lan_rate()` therefore never blocks a `true` write, and the write path (`ui/routes/instance_routes.py`) persists `lan_rate_enabled` exactly as submitted, with no backend clamping. `tests/test_lan_rate_policy.py::test_enabling_lan_rate_on_minqlxtended_is_never_blocked` pins this as deliberate, so a saved instance config round-trips instead of being silently rewritten. The stored field can still end up `true` on a minqlxtended instance: Load Preset is not gated by the same "never toggle it on" rule as the form control — `AddInstanceForm.jsx` and `EditInstanceConfigModal.jsx` only clamp a preset's `lan_rate_enabled: true` to `false` when the host does *not* support LAN rate, and that condition is never true for an always-on host, so the preset's value passes straight through into the submit payload. A direct API call or a bundle import can persist `true` the same way.
