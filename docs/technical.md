...
-   **Runtimes:** the toggle is not runtime-gated. `_build_qlds_args_string()` (`ui/task_logic/ansible_instance_mgmt.py`) sets `sv_lanForceRate` from `instance.lan_rate_enabled` for every instance on both runtimes, and has done since the initial public release. The `force_rate.so` exclusion on minqlxtended (see **Server Runtimes** above) is a separate concern — it avoids double-patching a function prologue minqlxtended patches itself — and does not gate this cvar.
