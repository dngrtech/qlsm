...
* **Frontend Features:**
    ...
    *   Host Logs (`/host-logs`, `/host-logs/:hostId`): read-only CodeMirror view of `Host.logs` served by `GET /api/hosts/<id>/logs`, as a standalone page with a host picker and as `components/hosts/HostLogsModal.jsx` opened from the host actions menu (state in `hooks/useHostLogsModal.js`). Both views guard their fetch with a request counter so only the newest response may write state, and the modal scopes its scroll-to-bottom to its own editor through a ref. `Host.logs` is unbounded and the endpoint returns it whole.
...
QLSM builds and runs one of three runtimes per host: **minqlx** (`MinoMino/minqlx`, the original), **minqlxtended** (`tjone270/minqlxtended`, a hard fork built unpatched), and **minqlxtended-patched** (the same fork, with QLSM's own qlhub patch chain applied — see `ansible/playbooks/tasks/build_engine_hook.yml`). minqlx and vanilla minqlxtended have no plugin compatibility in either direction; minqlxtended-patched deliberately shares minqlx's plugin pool and shared directory instead of getting its own (see the table below). `ui/runtime.py` is the single source of truth for every value that differs between them — nothing else in the backend or the Terraform roots hardcodes a runtime-specific path or pin (the Ansible playbooks re-derive the same values themselves from the `runtime` extra-var, since Ansible cannot import this module directly). Its API:

-   `MINQLX`, `MINQLXTENDED`, `MINQLXTENDED_PATCHED`, `VALID_RUNTIMES`, `DEFAULT_RUNTIME` (`'minqlx'`) — the constants.
...
| | minqlx | minqlxtended | minqlxtended-patched |
|---|---|---|---|
| Plugins dir (instance and `ql-assets/data/`) | `minqlx-plugins` | `minqlxtended-plugins` | `minqlx-plugins` (shared with minqlx) |
| Shared build dir | `/home/ql/minqlx-shared` | `/home/ql/minqlxtended-shared` | `/home/ql/minqlx-shared` (shared with minqlx) |
| Engine `.so` | `minqlx.x64.so` | `minqlxtended.x64.so` | `minqlxtended.x64.so` |
| Launch script | `run_server_x64_minqlx.sh` | `run_server_x64_minqlxtended.sh` | `run_server_x64_minqlxtended.sh` |
| Log filename | `minqlx.log` | `minqlxtended.log` | `minqlxtended.log` |
| Git repo / pin | `MinoMino/minqlx` @ `fbdd915185337791d8e209dc4b686a1ee60d3721` | `tjone270/minqlxtended` @ `97fbe6715a4802545aa7eca741d11e2486a306a4` | `tjone270/minqlxtended` @ `HEAD` (unpinned, see below) |
| Terraform OS image | Debian 12 x64 (bookworm) | Ubuntu 24.04 LTS x64 | Ubuntu 24.04 LTS x64 |
| Python floor | none (existing hosts run whatever they already run) | 3.12 (the build links `-lpython3.12`) | 3.12 |

`minqlxtended-patched`'s `git_version` is deliberately left floating on `HEAD` rather than pinned: there has never been a pin for this chain. Upstream minqlxtended restructured its source tree in its "v1.0.0" release, and the vendored qlhub patch scripts were verified only against the pre-restructure layout, so floating `HEAD` risks the patch chain failing loudly (acceptable) or applying wrongly (not) on a future upstream restructure. Pinning is a known follow-up, not an oversight.
...
**Host setup (`setup_host.yml`) and the shared engine build (`tasks/build_engine_hook.yml`):** both `setup_host.yml` and the manual `rebuild_minqlx.yml` delegate the actual build to the shared `tasks/build_engine_hook.yml`, which branches on the `runtime` extra-var. minqlx compiles with the local patches under `ql-assets/patches/*.patch` applied first; vanilla minqlxtended applies no patches and links `minqlxtended.zip` (the Python package, zipped) beside the compiled `.so` instead of an unpacked package directory; minqlxtended-patched additionally applies the 7-script qlhub patch chain vendored under `ql-assets/patches/minqlxtended/` (item events/respawn, native demo capture, a bounded redis pool, `set_position`) via `qlds-scripts/patch-minqlxtended-qlhub.py`. All three builds run under the `ql` system user and sync a runtime-appropriate `requirements.txt` from `ql-assets/data/<runtime>-plugins/`.
...
**Preset-only plugins and their native companions:** not every plugin QLSM ships comes from `ql-assets/data/`. `highfps.py`/`highfps_hook.so` and `footsteps.py`/`footsteps_hook.so` live in the separate [`dngrtech/qlsm_plugins`](https://github.com/dngrtech/qlsm_plugins) repo and ship only inside a preset's `scripts/`, on both runtimes — the baseline directories carry no binaries, and each hook has to sit beside the `.py` that `ctypes.CDLL`s it out of its own directory. Both ship disabled: absence from the preset's `checked_plugins.json` is the off switch, and there is no separate flag. Each `.so` is runtime-agnostic native code patching `qzeroded`/`qagamex64.so`, so there is one build per hook and both presets ship the identical binary; `tests/test_default_minqlxtended_preset.py` pins each by sha256 and asserts the two copies match, so rebuilding one alone fails CI. A preset declares each such binary in `preset.json` under `binary_descriptions` (`"scripts/<name>.so"` → operator-facing text); `_validate_binary_descriptions()` in `ui/builtin_presets.py` rejects the whole preset at seed time if a declared key has no file behind it, so a stale entry takes the preset out entirely rather than degrading it. The two ends of that pipe address the same file differently, and the seeder reconciles them: manifest keys are **preset-root relative** so validation can resolve them on disk, while the `BinaryMetadata` row is filed **relative to `scripts/`** because the plugin file manager builds its tree with `scripts/` as the base (`_build_draft_tree()`) and `get_binary_meta()` matches `file_path` as an exact string. `_binary_row_path()` strips the leading `scripts/` and preserves everything below it. Skip that strip and the description sits correctly in the table while the UI renders an empty box.
...
-   **Playbook Structure:**
    -   **`ansible/playbooks/setup_host.yml`:** Performs the initial one-time setup on a newly provisioned host. Installs prerequisites (including `iptables-persistent`, and `redis-server` only when the host runtime needs its own Redis), configures the firewall using a template (`ansible/templates/iptables.rules.j2`) that defines both filter and NAT rules which are applied atomically via `iptables-restore` and persisted, creates the `ql` user, installs base SteamCMD/QLDS, delegates the shared engine build to `tasks/build_engine_hook.yml` (see **Server Runtimes** above), and syncs common assets (`/home/ql/assets/common`). Building minqlxtended or minqlxtended-patched asserts the host's Python is 3.12+ before compiling.
    -   **`ansible/playbooks/rebuild_minqlx.yml`:** Rebuilds the selected runtime's engine hook from source (or fetches a prebuilt artifact) and redeploys it to its shared location without a full host setup, via `tasks/build_engine_hook.yml` — the same task used by `setup_host.yml`. Run manually with `-e '{"runtime": "..."}'`.
