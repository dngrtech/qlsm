...
-   **minqlxtended hosts:** `lan_rate_always_on()` (`ui/lan_rate_policy.py`) reports the toggle as unconditionally on, since minqlxtended hooks `Sys_IsLANAddress` itself and excludes `force_rate.so` (see **Server Runtimes** above) — neither the hook path nor the legacy NAT path is reachable there. `host_supports_lan_rate()` treats an always-on host as supported, so `would_enable_unsupported_lan_rate()` never blocks a `true` write for minqlxtended — the write path (`ui/routes/instance_routes.py`) persists `lan_rate_enabled` exactly as submitted, with no backend clamping; `tests/test_lan_rate_policy.py::test_enabling_lan_rate_on_minqlxtended_is_never_blocked` pins that this is deliberate, so a saved instance config round-trips instead of being silently rewritten. What's forced is UI-side only, and only against direct operator input: QLSM's own forms lock the toggle on-and-disabled and never let an operator switch it on themselves. Loading a preset that already carries `lan_rate_enabled: true` is a different path and is not clamped — both `AddInstanceForm.jsx` and `EditInstanceConfigModal.jsx` only zero out a preset's `true` when the host does *not* support LAN rate, so on an always-on minqlxtended host it passes straight through into the submit payload (deliberately — clamping it here would spuriously mark the preset "modified" and let Overwrite Preset silently strip the setting). A direct API call or a bundle import can persist `true` the same way. `frontend-react/src/utils/lanRateCompatibility.js` mirrors the always-on *display* policy for the frontend — the two files are deliberate mirrors of each other for that purpose; see [LAN Rate Policy Mirrors](development_guidelines.md#lan-rate-policy-mirrors) in Development Guidelines.
