...
*   **RCON Service (`rcon_service/`):** A set of modules providing remote console and live stats access over ZMQ. Used by the frontend's RCON console modal, the Global RCON page, and live status polling. `ConnectionManager` tracks RCON connections by `(host_id, instance_id)` and, separately, tracks which connection owns each physical stats endpoint `(ip, zmq_stats_port)` — a recreated instance keeps its game port and therefore its stats port, so the second registry is what stops a stale record from holding the same socket open with an outdated password. Stats passwords are logged only as a truncated SHA-256 fingerprint. See [RCON Integration](rcon_integration.md).
