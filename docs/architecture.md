...
*   **SQLite DB:** A simple file-based database storing application metadata:
    ...
    * **Operator Model:** A directory of named SteamID64s, with a default admin level, used by the Owner & Admins panel to write `qlx_owner` (`server.cfg`) and `steamid|level` lines (`access.txt`). It isn't linked to any instance: the config files stay the source of truth. After each deploy and config apply, `access_permission_sync.py` pushes the `access.txt` levels into the instance's minqlx Redis permission keys over SSH (see `docs/api_reference.md` → In-Game Permission Sync).
...
*   **Plugin Pool as Source of Truth:** `ql-assets/data/minqlx-plugins/` is the authoritative copy of every plugin QLSM ships — it's what `setup_host.yml` deploys as the host baseline, what `ui/task_logic/backup_files.py` backs up, and what `ui/plugin_manifest.py` reads `*.ql-plugin.json` metadata from regardless of where a `.py` file physically lives (pool entry always wins over a preset/instance sidecar). All plugin logic edits happen in the pool, not in `configs/presets/_builtin/default/scripts/` (the builtin "default" `ConfigPreset`) or any instance's own copy. The *default preset's* copy can otherwise silently drift from the pool, since nothing used to sync file *content* between them (manifest lookup already preferred the pool, but `.py` code did not) — `ui/plugin_pool_sync.py` (`flask sync-plugin-pool`, `--check` for CI) mirrors the pool onto the builtin default preset's `scripts/` dir, and `tests/test_plugin_pool_preset_sync.py` fails the suite if they diverge again. The one deliberate exception is `highfps.py`/`highfps_hook.so`, a togglable per-preset LD_PRELOAD hook, not a pool plugin. Line endings for both trees are pinned to LF via `.gitattributes` so a CRLF/LF-only diff can't masquerade as content drift.
