...
| --- | --- | --- | --- |
| `v1.12.9` | 2026-06-21 | [#124](https://github.com/dngrtech/qlsm/pull/124) | `reset_acc` plugin now privately shows a player their overall accuracy (`^7ACC: ^2[n]%`) on every kill and death while `!autoresetstats` is enabled, so they see what they had right before the full-stats wipe. Required a new `player_accuracy()` minqlx C binding (returns `(hits, shots)`) added to `minqlx-reset-accuracy.patch`; rebuild/redeploy minqlx via re-run host setup to activate. |
| `v1.12.8` | 2026-06-21 | [#123](https://github.com/dngrtech/qlsm/pull/123) | Hide the `user-hooks/` directory from the Configuration Files tab. It was leaking into the config-folder listing because only `scripts` and `factories` were treated as reserved; `user-hooks` has its own dedicated Hooks tab and should not appear as a managed config folder. Added `user-hooks` to `RESERVED_CONFIG_FOLDER_NAMES` in both `instance_routes.py` and `preset_api_routes.py`. |
| `v1.12.7` | 2026-06-20 | [#122](https://github.com/dngrtech/qlsm/pull/122) | Add `qlx_brandingMapCredit` cvar to `branding.py` (default `0`). When `0`, the map's baked-in author credit (e.g. "Till Merker") is dropped from the loading screen and only the configured `qlx_serverBrandTopField` / `qlx_serverBrandBottomField` brand text is shown; set to `1` to restore the original behaviour of prepending the map credit. Synced to all `configs/presets/` copies and the `ql-assets/` canonical copy. |
