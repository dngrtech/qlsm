...
| --- | --- | --- | --- |
| `v1.12.9` | 2026-06-21 | [#124](https://github.com/dngrtech/qlsm/pull/124) | `reset_acc`: privately show a player their overall accuracy (`^7ACC: ^2[n]%`) on each kill/death while `!autoresetstats` is on, so they see it before the wipe. Adds a `player_accuracy()` minqlx C binding to `minqlx-reset-accuracy.patch`; rebuild/redeploy minqlx (re-run host setup) to activate. |
