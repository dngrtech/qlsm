...
| --- | --- | --- | --- |
| `v1.11.4` | 2026-06-11 | [#TBD](https://github.com/dngrtech/qlsm/pull/TBD) | Prevent `needrestart` from restarting `systemd-networkd` and `systemd-udevd` after package upgrades. On OVH VPS hosts, restarting these services disrupts the /32 DHCP routing setup and causes the VM to become unreachable for incoming connections until rebooted. |
