...
| --- | --- | --- | --- |
| `v1.12.9` | 2026-06-21 | [#PR](https://github.com/dngrtech/qlsm/pull/PR) | `reset_acc` plugin now privately shows a player their overall accuracy (`^7ACC: ^2[n]%`) on every kill and death while `!autoresetstats` is enabled, so they see what they had right before the full-stats wipe. Required a new `player_accuracy()` minqlx C binding (returns `(hits, shots)`) added to `minqlx-reset-accuracy.patch`; rebuild/redeploy minqlx via re-run host setup to activate. |
