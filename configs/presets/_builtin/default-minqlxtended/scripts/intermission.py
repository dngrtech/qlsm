...
class intermission(minqlxtended.Plugin):
    def __init__(self):
        self.index = 0
    ...
    def cmd_intermission( self, player, msg, channel):
        for p in self.players():
            if self.db.get_flag(p, "essentials:sounds_enabled", default=True):
                self.stop_sound(p)
        self.handle_game_end( False )  # was: self.handle_game_end({"ABORTED": 0}) - game_end now dispatches a plain aborted bool instead of a stats dict
