...
class commlink(minqlxtended.Plugin):
    def __init__(self):
        ...
        self.add_command(("world", "say_world"), self.send_commlink_message, priority=minqlxtended.Priority.LOWEST, usage="<message>")
    ...
    def handle_msg(self, irc, user, channel, msg):
        def broadcast_commlink(pm):
            ...
            minqlxtended.console_print("[CommLink] ^5{}^7:^3 {}".format(user[0], " ".join(pm)))
    ...
    def query_status(self):
        ...
        status = self.get_status_msg()
        minqlxtended.console_print("[CommLink] ^5{}^7: {}".format(self.clientName, status))
        for p in self.players():
...
class SimpleAsyncIrc(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            minqlxtended.CHAT_CHANNEL.reply("Disconnected from ^3CommLink^7. Reconnecting in 30 seconds...")
            time.sleep(30)
        loop.close()
