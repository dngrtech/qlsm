...
try:
    ...
    try:
        ...
        minqlxtended.CHAT_CHANNEL.reply("^1iouonegirl abstract plugin download failed^7: {}".format(e))
        raise
...
class mybalance(iouonegirlPlugin):
    def __init__(self):
        ...
        self.add_hook("player_connect", self.handle_player_connect, priority=minqlxtended.Priority.HIGH)
        self.add_hook("player_disconnect", self.handle_player_disconnect)
        self.add_hook("game_countdown", self.handle_game_countdown)
        self.add_hook("new_game", self.handle_new_game)
        self.add_hook("vote_called", self.handle_vote_called)

        self.add_command(("setrating", "setelo"), self.cmd_setrating, 3, priority=minqlxtended.Priority.HIGH, usage="<id>|<name> <rating>")
        self.add_command(("getrating", "getelo", "elo"), self.cmd_getrating, priority=minqlxtended.Priority.HIGH, usage="<id>|<name> [gametype]")
        self.add_command(("remrating", "remelo"), self.cmd_remrating, 3, priority=minqlxtended.Priority.HIGH, usage="<id>|<name>")
    ...
    def cmd_help_load_exceptions(self, player, msg, channel):
        ...
        try:
            ...
            try:
                ...
                minqlxtended.CHAT_CHANNEL.reply("^6mybalance plugin^7: No exception list found, so I made one myself.")
            except:
                minqlxtended.CHAT_CHANNEL.reply("^1Error: ^7reading and creating exception list: {}".format(e))

        except Exception as e:
            minqlxtended.CHAT_CHANNEL.reply("^1Error: ^7reading exception list: {}".format(e))
    ...
    def check_warmup(self, warmup, mapname):
        while self.is_game_in_warmup() and self.game_with_map_loaded(mapname) and self.warmup_reminders and \
            ...
            if diff >= int(self.get_cvar('qlx_mybalance_warmup_seconds')):
                pgs = minqlxtended.Plugin._loaded_plugins
    ...
    def is_plugin_still_loaded(self) -> bool:
        return self.__class__.__name__ in minqlxtended.Plugin._loaded_plugins
    ...
    def handle_game_countdown(self):
        ...
        if 'balance' in minqlxtended.Plugin._loaded_plugins:
            self.msg("^7Balancing on skill ratings...")
            b = minqlxtended.Plugin._loaded_plugins['balance']
            teams = self.teams()
            players = dict([(p.steam_id, self.game.type_short) for p in teams["red"] + teams["blue"]])
            b.add_request(players, b.callback_balance, minqlxtended.CHAT_CHANNEL)
        else:
    ...
    def handle_player_connect(self, player, is_bot):  # minqlxtended passes is_bot too; unused here
        ...
        if not (player.steam_id in self.exceptions or self.db.has_permission(player, self.get_cvar("qlx_mybalance_perm_allowed", int))):
            ...
            try:
                ...
                minqlxtended.console_command("echo MybalanceError: {}".format(e))
    ...
    def evaluate_team_balance(self, player=None):
        ...
        def setpos(_p, _x, _y, _z):
            _p.position = (_x, _y, _z)  # position/velocity are settable properties on minqlxtended, not methods
            _p.velocity = (0, 0, 0)
        @minqlxtended.next_frame
        def cprint(_p, _m):
    ...
    def balance_before_start(self, roundnumber, direct=False):
        @minqlxtended.next_frame # Game logic should never be done in a thread directly
        ...
        if self.game.type_short == "ca":
            if self.game.roundlimit in [self._team_score(minqlxtended.Team.BLUE), self._team_score(minqlxtended.Team.RED)]:
                return

        if self.game.type_short == "tdm":
            if self.game.fraglimit in [self._team_score(minqlxtended.Team.BLUE), self._team_score(minqlxtended.Team.RED)]:
                return

        if self.game.type_short == "ctf":
            if self.game.capturelimit in [self._team_score(minqlxtended.Team.BLUE), self._team_score(minqlxtended.Team.RED)]:
                return
        ...
        while abs(red_min_blue(excluded_teams)) >= 1:
            ...
            if not last:
                ...
                minqlxtended.console_command("echo Error: Trying to balance before round {} start. Red({}) - Blue({}) players".format(roundnumber, len(teams['red']), len(teams['blue'])))
                return
            if self.is_even(diff): # one team has an even amount of people more than the other
                ...
                if self.prevent or self.last_action == "ignore":
                    ...
                    if 'anti_rape' in minqlxtended.Plugin._loaded_plugins:
                        game_logic(lambda: last.put("spectator"))
                        self.msg("^6Uneven teams action^7: {} was moved to spec to even teams!".format(last.name))
                        minqlxtended.console_command("echo Not slayed because anti_rape plugin is loaded.")
                    else:
                        slay_player(last)
    ...
    def cmd_last_action(self, player, msg, channel):
        if len(msg) < 2:
            if self.last_action == 'slay' and 'anti_rape' in minqlxtended.Plugin._loaded_plugins:
    ...
    # next round starts
    def handle_round_end(self, round_number, winning_team, round_time):  # minqlxtended round_end signature; none of these were used
        self.prevent = False
...
##                self.balance_before_start(round_number, True)
    ...
    def handle_game_start(self):  # minqlxtended's game_start dispatches no arguments; the old "data" was unused
    ...
    def cmd_setrating(self, player, msg, channel):
        ...
        try:
            ...
            if not target_player: return minqlxtended.Return.STOP_ALL
    ...
    def _team_score(self, team):
        # Game.red_score / Game.blue_score don't exist on minqlxtended. Scores come off
        # team_scores, indexed by Team.index. Guarded because team_scores reads the live
        # level and raises NonexistentGameError once the game is gone.
        try:
            return self.game.team_scores[team.index]
    ...
    def algo_get_last(self, excluded_teams = False):
        ...
        if len(teams["blue"]) > len(teams["red"]):
            ...
            minqlxtended.console_command("echo Cannot pick last player since there are none.")
            return

        if (self._team_score(minqlxtended.Team.RED) + self._team_score(minqlxtended.Team.BLUE)) >= 1:

            minqlxtended.console_command("echo Picking someone to {} based on score".format(self.last_action))
            ...
            minqlxtended.console_command("echo Picking someone to {} based on join times.".format(self.last_action))
            bigger_team.sort(key = lambda el: self.find_time(el), reverse=True)
            lowest_player = bigger_team[0]

        minqlxtended.console_command("echo Picked {} from the {} team.".format(lowest_player.name, lowest_player.team))
        ...
        while attempts < MAX_ATTEMPTS:
            ...
            for p in js["players"]:
                ...
                if _sid == sid: # got our player
                    if gt not in p:
                        minqlxtended.console_command("echo No {} rating for {}".format(gt, _sid))
    ...
    def callback_elo(self, player, gt, elo = 0, games=0):
        ...
        if elos: minqlxtended.CHAT_CHANNEL.reply("^6{}".format(m) + " ^7, ".join(elos) + "^7.")
    ...
    @minqlxtended.delay(600) # 10 minutes
