...
class serverBDM(minqlxtended.Plugin):
    def __init__(self):
        if ENABLE_LOG:
        ...
        self.add_hook("vote_called", self.handle_vote_called, priority=minqlxtended.Priority.HIGH)
        ...
        self.add_command("bbalance", self.cmd_bdmbalance, int(minqlxtended.get_cvar("qlx_bdmAdmin")))
        self.add_command("balance", self.balance_cmd, int(minqlxtended.get_cvar("qlx_bdmAdmin")))
        self.add_command(("agree", "a"), self.cmd_bdmagree)
        self.add_command("do", self.cmd_bdmdo, int(minqlxtended.get_cvar("qlx_bdmAdmin")))
        self.add_command("setbdm", self.cmd_set_bdm, int(minqlxtended.get_cvar("qlx_bdmAdmin")))
        self.add_command("prebalance", self.cd_bdmbalance, int(minqlxtended.get_cvar("qlx_bdmAdmin")))
        self.add_command("damage", self.cmd_damage_status)
        self.add_command("dmg", self.cmd_dmg_status)
        self.add_command("gamestatus", self.cmd_game_status, int(minqlxtended.get_cvar("qlx_bdmAdmin")))
        self.add_command("mark_agree", self.cmd_mark_agree, int(minqlxtended.get_cvar("qlx_bdmAdmin")))
        ...
        self._countdown_time = int(minqlxtended.get_cvar("g_roundWarmupDelay")) / 1000
        ...
        self._short_ft_countdown = self._bdm_gtype == "ft" and int(minqlxtended.get_cvar("g_freezeRoundDelay")) <= 1000
        ...
        if bool(int(minqlxtended.get_cvar("qlx_bdmBalanceAtGameStart"))) and\
                bool(int(minqlxtended.get_cvar("qlx_bdmAllowUnevenTeamsReady"))):
            minqlxtended.console_command('set g_teamForceBalance "0"')
        else:
            minqlxtended.console_command('set g_teamForceBalance "1"')
    ...
    def handle_chat(self, player, msg, channel, recipient):
        # minqlxtended dispatches a 4th "recipient" arg (None for public chat); unused here,
        # same as the original minqlx 3-arg behaviour which only ever handled public chat.
        ...
        try:
            if "what is bdm" in msg.lower():
                channel.reply("^6BDM ^7is ^6B^7asic ^6D^7amage ^6M^7etric. It is a server side skill calculation "
                              ...
                              .format(minqlxtended.get_cvar("qlx_commandPrefix")))
    ...
    def handle_player_connect(self, player, is_bot):
        # minqlxtended dispatches a 2nd "is_bot" arg; unused, this plugin already detects
        # bots itself via the steam_id[0] == "9" convention used throughout.
    ...
    def process_player_connect(self, player):
        try:
            ...
            if sid[0] == "9":
                return
            factory = minqlxtended.get_cvar("g_factory").lower()
            if factory == "ictf":
                game_type = "ictf"
            ...
            self.set_bdm_field(player, game_type, "rating", minqlxtended.get_cvar("qlx_bdmDefaultBDM"), False)
            self.set_bdm_field(player, game_type, "games_completed", "0", False)
            self.set_bdm_field(player, game_type, "games_left", "0", False)
            if bool(int(minqlxtended.get_cvar("qlx_bdmSetIntitialBDM"))) and\
    ...
    def process_player_loaded(self, player):
        try:
            if bool(int(minqlxtended.get_cvar("qlx_bdmPrintJoinMsg"))):
                sid = str(player.steam_id)
                if sid[0] != "9":
                    if bool(int(minqlxtended.get_cvar("qlx_bdmPrintBdmStatsEveryMap"))):
    ...
    def handle_round_end(self, round_number, winning_team, round_time):
        # minqlxtended dispatches (round_number, winning_team, time) instead of a stats
        # dict; only round_number was ever used (as data["ROUND"]). winning_team/round_time
        # (renamed from "time" to avoid shadowing the top-level "time" module) are unused.
        self._between_rounds = True
        self.process_round_end(round_number)
        return

    def process_round_end(self, round_number):
        try:
            self.rounds_played = int(round_number)
            ...
            if self._bdm_gtype in TEAM_BASED_GAMETYPES:
                minqlxtended.console_print("^1RED^7: ^7{} ^6::: ^4BLUE^7: {}"
                                     .format(minqlxtended.configstring(6), minqlxtended.configstring(7)))
            if (self._locked[0] or self._locked[1]) and time.time() - self._balance_time > 5:
                minqlxtended.console_command("unlock red")
                self._locked[0] = False
                minqlxtended.console_command("unlock blue")
    ...
    def handle_game_start(self):
        # minqlxtended dispatches no arguments; the old "data" stats dict was never
        # referenced in this handler anyway.
        ...
        try:
            if ENABLE_LOG:
                ...
                if factory == "ictf":
                    ...
                    game_type = self._bdm_gtype
                self.bdm_log.info("New Game starting {}".format(game_type))
            self.players_in_teams()
            if self._bdm_gtype == "ft" and int(minqlxtended.get_cvar("g_freezeRoundDelay")) < 1000:
                return
            if bool(int(minqlxtended.get_cvar("qlx_bdmBalanceAtGameStart"))) and self._bdm_gtype in TEAM_BASED_GAMETYPES and\
    ...
    def handle_game_end(self, aborted):
        # minqlxtended dispatches the bool directly instead of a data["ABORTED"] stats dict.
        if aborted:
            self.game_start = 0
            self.reset_data()
    ...
    def game_end_thread(self, game_type, game_time):
        num = Value('i', 0)
        old = Array('i', range(64))
        new = Array('i', range(64))
        steam_id = Array(c_longlong, range(64))
        p = Process(target=self.process_game_end, args=(num, steam_id, old, new, game_type, game_time,))
        p.start()
        p.join()
        self.print_changes(num, steam_id, old, new)
        return

    def process_game_end(self, num, steam_id, old, new, game_type, game_time):
        try:
            time.sleep(0.5)
            self.process_game_data(num, steam_id, old, new, game_type, game_time)
        except Exception as e:
            if ENABLE_LOG:
                self.bdm_log.info("serverBDM process_game_end Exception: {}".format([e]))

    def handle_vote_called(self, caller, vote, args):
        minqlxtended.console_print("{} ^7called vote: ^6{} {}".format(caller, vote, args))
        if vote.lower() == "bbalance" or\
                vote.lower() == "balance" and bool(int(minqlxtended.get_cvar("qlx_bdmRespondToBalanceCommand"))):
            if self.game.state in ["in_progress", "countdown"]:
                self.msg("^3Game is active. Use {}{} to check team balance."
                         .format(minqlxtended.get_cvar("qlx_commandPrefix"),
                                 "teams" if bool(int(minqlxtended.get_cvar("qlx_bdmRespondToTeamsCommand"))) else "bteams"))
                return minqlxtended.Return.STOP_ALL
            self.callvote("qlx {}bbalance".format(minqlxtended.get_cvar("qlx_commandPrefix")),
            ...
            if self.game.state in ["in_progress", "countdown"]:
                ...
                return minqlxtended.Return.STOP_ALL
            elif bool(int(minqlxtended.get_cvar("qlx_bdmBalanceAtGameStart"))) and self._bdm_gtype in BDM_GAMETYPES:
                self.msg("^3Shuffle vote denied. Teams ^4will be balanced ^3at start of game.")
                return minqlxtended.Return.STOP_ALL
        elif vote.lower() == "do" and bool(int(minqlxtended.get_cvar("qlx_bdmEnableDoVote"))) and\
    ...
    def process_vote_ended(self, vote, passed):
        try:
            self.vote_count[2] = 0
            if passed and vote.lower() == "shuffle" and bool(int(minqlxtended.get_cvar("qlx_bdmBalanceAfterShuffleVote"))):
                if self._bdm_gtype in TEAM_BASED_GAMETYPES:

                    @minqlxtended.delay(2.5)
                    def exec_balance():
    ...
    def respond_to_elo_requests(self, player, msg, channel):
        if int(minqlxtended.get_cvar("qlx_bdmModifyEloCmds")) == 2:
    ...
    def cmd_set_bdm(self, player, msg, channel):
        try:
            ...
            if not target_player:
                ...
                return minqlxtended.Return.STOP_ALL
            if rating > int(minqlxtended.get_cvar("qlx_bdmMaxRating")) or rating < int(minqlxtended.get_cvar("qlx_bdmMinRating")):
    ...
    def teams_cmd(self, player, msg, channel):
        if bool(int(minqlxtended.get_cvar("qlx_bdmRespondToTeamsCommand"))):
        ...
        try:
            if self._bdm_gtype in TEAM_BASED_GAMETYPES:
                ...
                if abs(suggestion[3]) < abs(suggestion[2]) and\
                        abs(suggestion[2]) >= int(minqlxtended.get_cvar("qlx_bdmMinimumSuggestionDiff")):
                    ...
                    self.msg("^6Switch ^1::^7{}^1::^7<-> ^4::^7{}^4:: ^6{}a ^7to agree."
                             .format(suggestion[0].name, suggestion[1].name, minqlxtended.get_cvar("qlx_commandPrefix")))
                    diff = int(minqlxtended.configstring(6)) - int(minqlxtended.configstring(7))
                ...
                if all(self._players_agree):
                    ...
                    if self.game.state in ["in_progress", "countdown"]:
                        if self._bdm_gtype in ROUND_BASED_GAMETYPES:
                            ...
                            if bool(int(minqlxtended.get_cvar("qlx_bdmRequireDoVote"))) and\
                                    bool(int(minqlxtended.get_cvar("qlx_bdmEnableDoVote"))):
    ...
    def place_player(self, player_id, team):
        try:
            minqlxtended.console_command("put {} {}".format(player_id, team))
        except Exception as e:
            if ENABLE_LOG:
                self.bdm_log.info("serverBDM place_player Exception: {}".format([e]))

    def balance_cmd(self, player, msg, channel):
        if bool(int(minqlxtended.get_cvar("qlx_bdmRespondToBalanceCommand"))):
    ...
    def exec_bdmbalance(self, pause):
        ...
        try:
            ...
            if bool(int(minqlxtended.get_cvar("qlx_bdmLockTeamsForBalance"))):
                if ENABLE_LOG:
                    self.bdm_log.info("Locking teams to perform BDM balance.")
                minqlxtended.console_command("lock red")
                self._locked[0] = True
                minqlxtended.console_command("lock blue")
                self._locked[1] = True
            ...
            if (len(teams["red"]) + len(teams["blue"])) % 2 != 0:
                if bool(int(minqlxtended.get_cvar("qlx_bdmBalanceUnevenTeams"))):
                    try:
                        exclude = self.plugins["specqueue"].return_spec_player((teams["red"] + teams["blue"]))[0]
                    except KeyError:
                        lowest = int(minqlxtended.get_cvar("qlx_bdmMaxRating"))
                        for player in (teams["red"] + teams["blue"]):
    ...
    def exec_game_status(self):
        try:
            minqlxtended.console_print("^6Game Status: ^4Map ^1- ^7{} ^5Game Mode ^1- ^7{}"
                                 .format(minqlxtended.get_cvar("mapname"), self._bdm_gtype.upper()))
            teams = self.teams()
            if len(teams["red"] + teams["blue"] + teams["free"] + teams["spectator"]) == 0:
                minqlxtended.console_print("^3No players connected")
            else:
                if self.game.state not in ["in_progress", "countdown"]:
                    minqlxtended.console_print("^3Match is not in progress")
                if self._bdm_gtype in TEAM_BASED_GAMETYPES:
                    red_team = []
                    blue_team = []
                    minqlxtended.console_print("^1RED^7: ^7{} ^6::: ^4BLUE^7: {}"
                    ...
                    for player in teams["red"]:
                        red_team.append("    ^1{} ^5{} ^6{} ^7{} {} ^2{}^7/^1{} ^2{}^7/^1{}"
                                        .format(player.id, player.stats.ping, player.stats.score, player,
                                                ...
                                                player.stats.kills, player.stats.deaths, player.stats.damage_dealt,
                                                player.stats.damage_taken))
                    minqlxtended.console_print("^1Red^7: ^7(^1ID ^5Ping ^6Score ^7Name ^2Kills^7/^1Deaths"
                                         " ^2DmgDlt^7/^1DmgTkn^7) ^1{} ^7Players".format(len(red_team)))
                    for player in red_team:
                        minqlxtended.console_print(player)
                    for player in teams["blue"]:
                        blue_team.append("    ^4{} ^5{} ^6{} ^7{} {} ^2{}^7/^1{} ^2{}^7/^1{}"
                                         .format(player.id, player.stats.ping, player.stats.score, player,
                                                 ...
                                                 player.stats.kills, player.stats.deaths, player.stats.damage_dealt,
                                                 player.stats.damage_taken))
                    minqlxtended.console_print("^4Blue^7: ^7(^4ID ^5Ping ^6Score ^7Name ^2Kills^7/^1Deaths"
                                         " ^2DmgDlt^7/^1DmgTkn^7) ^4{} ^7Players".format(len(blue_team)))
                    for player in blue_team:
                        minqlxtended.console_print(player)
                else:
                    for player in teams["free"]:
                        minqlxtended.console_print("{}^7: {} ^6Ping^7: {}"
                ...
                for player in teams["spectator"]:
                    if specq:
                        ...
                        slot = False
                    minqlxtended.console_print("^6Spectator^7: {} {} {}"
                                         .format(player.id, player,
    ...
    def remove_conflicting_commands(self):
        teams = bool(int(minqlxtended.get_cvar("qlx_bdmRespondToTeamsCommand")))
        balance = bool(int(minqlxtended.get_cvar("qlx_bdmRespondToBalanceCommand")))
        elo = int(minqlxtended.get_cvar("qlx_bdmModifyEloCmds"))
        loaded_scripts = self.plugins
        loaded_scripts.pop(self.__class__.__name__)
        if elo in [1, 2] and "balance" in loaded_scripts:
            minqlxtended.unload_plugin("balance")
            loaded_scripts.pop("balance")
            minqlxtended.console_print("^1serverBDM: Unloading the ^7balance ^1plugin")
            if ENABLE_LOG:
                self.bdm_log.info("serverBDM: Unloading the balance plugin")
        for script, handler in loaded_scripts.items():
            try:
                for cmd in handler.commands:
                    # minqlxtended's Command.name is only the primary alias (a str); the old
                    # minqlx Command.name held every alias (a list), which is what the set
                    # intersections below need. Command.names is the equivalent list here.
                    if (teams and {"teams", "a", "do"}.intersection(cmd.names)) or \
                            (balance and {"balance"}.intersection(cmd.names)) or \
                            (elo == 1 and {"elo", "elos"}.intersection(cmd.names)):
                        handler.remove_command(cmd.names, cmd.handler)
                        minqlxtended.console_print("^1serverBDM: Removing command ^7{} ^1used in ^7{} ^1plugin"
                                             .format(cmd.names, script))
                        if ENABLE_LOG:
    ...
    def display_join_message(self, player):
        ...
        try:
            ...
            bdm_rating = int(minqlxtended.get_cvar("qlx_bdmDefaultBDM"))
            if ENABLE_LOG:
        ...
        join_msg = minqlxtended.get_cvar("qlx_bdmJoinMessage").replace("%", "％")
        try:
    ...
    def check_force_switch_vote(self, caller, vote):
        try:
            self.callvote("qlx {}mark_agree".format(minqlxtended.get_cvar("qlx_commandPrefix")),
                          "Force the suggested player switch?")
            minqlxtended.client_command(caller.id, "vote yes")
            self.msg("{}^7 called vote /cv {}".format(caller.name, vote))
            voter_perc = int(minqlxtended.get_cvar("qlx_bdmPlayerPercForVote"))
            if voter_perc > 0:
                self.vote_count[0] = 1
                self.vote_count[1] = 0
    ...
    def default_bdm(self, player):
        try:
            if str(player.steam_id)[0] == "9":
                ...
                player_bdm = int(minqlxtended.get_cvar("qlx_bdmDefaultBDM"))
            return player_bdm
        except Exception as e:
            if ENABLE_LOG:
    ...
    def execute_switch(self, delay=False):
        try:
            if delay:
                time.sleep(3)
            if not bool(int(minqlxtended.get_cvar("qlx_bdmEnableSwitch"))):
    ...
    def process_game_data(self, num, steam_id, o, n, game_type, game_time):
        try:
            match_time = 0
            self.game_active = False
            self._save_previous_bdm = {}
            ...
            match_perc = int(minqlxtended.get_cvar("qlx_bdmMinTimePerc")) / 100.0
            if game_type in TEAM_BASED_GAMETYPES:
                self.player_count = 0
                if game_type in ROUND_BASED_GAMETYPES:
                    if game_type == "wipeout":
                        min_rounds = int(minqlxtended.get_cvar("qlx_bdmWoMinRounds"))
                    else:
                        min_rounds = int(minqlxtended.get_cvar("qlx_bdmMinRounds"))
                    if self.rounds_played < min_rounds:
                        if ENABLE_LOG:
                            num.value = -1
                            self.bdm_log.info("--- Only {} rounds played but need {} rounds,"
                                              " BDM is not being calculated ---"
                                              .format(self.rounds_played, int(minqlxtended.get_cvar("qlx_bdmMinRounds"))))
                        return
                ...
                if not finished_calc:
                    ...
                    return
                if self.player_count < (int(minqlxtended.get_cvar("qlx_bdmMinimumTeamSize")) * 2):
                    if ENABLE_LOG:
                        num.value = -2
                        self.bdm_log.info("--- There are only {} players on each team, {} are needed. "
                                          ...
                                          .format(int(round(self.player_count / 2)),
                                                  int(minqlxtended.get_cvar("qlx_bdmMinimumTeamSize"))))
                    return
            ...
            self.calc_new_bdm(game_type, num, steam_id, o, n)
        except Exception as e:
            if ENABLE_LOG:
    ...
    def calc_ffa_dmga(self, needed_time, match_time):
        try:
            per_kill_pts = int(minqlxtended.get_cvar("qlx_bdmFfaKillPts"))
    ...
    def calc_ca_dmga(self, needed_time, match_time):
        try:
            per_kill_pts = int(minqlxtended.get_cvar("qlx_bdmCaKillPts"))
            dmg_rcvd_perc = int(minqlxtended.get_cvar("qlx_bdmDamageRcvdPerc")) / 100.0
            for sid in self._disconnected_players:
                if self._disconnected_players[sid]["time"] >= needed_time:
                    self.player_count += 1
                    self._player_stats[sid] = {}
    ...
    def calc_wipeout_dmga(self, needed_time, match_time):
        try:
            per_kill_pts = int(minqlxtended.get_cvar("qlx_bdmWoKillPts"))
            per_death_pts = int(minqlxtended.get_cvar("qlx_bdmWoDeathPts"))
            for sid in self._disconnected_players:
                if self._disconnected_players[sid]["time"] >= needed_time:
                    self.player_count += 1
                    self._player_stats[sid] = {}
    ...
    def calc_ft_dmga(self, needed_time, match_time):
        try:
            per_kill_pts = int(minqlxtended.get_cvar("qlx_bdmFtKillPts"))
            per_thaw_pts = int(minqlxtended.get_cvar("qlx_bdmFtThawPts"))
            per_frozen_pts = int(minqlxtended.get_cvar("qlx_bdmFtFrozenPts"))
            for sid in self._disconnected_players:
                if self._disconnected_players[sid]["time"] >= needed_time:
                    self.player_count += 1
                    self._player_stats[sid] = {}
    ...
    def calc_ctf_dmga(self, needed_time, match_time, game_type):
        try:
            dmg_rcvd_perc = int(minqlxtended.get_cvar("qlx_bdmDamageRcvdPerc")) / 100.0
            if game_type == "ictf":
                per_kill_pts = int(minqlxtended.get_cvar("qlx_bdmICtfKillPts"))
                per_cap_pts = int(minqlxtended.get_cvar("qlx_bdmICtfCapPts"))
                per_assist_pts = int(minqlxtended.get_cvar("qlx_bdmICtfAssistPts"))
                per_defense_pts = int(minqlxtended.get_cvar("qlx_bdmICtfDefensePts"))
            else:
                per_kill_pts = int(minqlxtended.get_cvar("qlx_bdmCtfKillPts"))
                per_cap_pts = int(minqlxtended.get_cvar("qlx_bdmCtfCapPts"))
                per_assist_pts = int(minqlxtended.get_cvar("qlx_bdmCtfAssistPts"))
                per_defense_pts = int(minqlxtended.get_cvar("qlx_bdmCtfDefensePts"))
    ...
    def calc_tdm_dmga(self, needed_time, match_time):
        try:
            per_kill_pts = int(minqlxtended.get_cvar("qlx_bdmTdmKillPts"))
            per_death_pts = int(minqlxtended.get_cvar("qlx_bdmTdmDeathPts"))
            for sid in self._disconnected_players:
                if self._disconnected_players[sid]["time"] >= needed_time:
                    self.player_count += 1
                    self._player_stats[sid] = {}
    ...
    def calc_new_bdm(self, game_type, num, steam_id, old, new):
        try:
            min_rating = int(minqlxtended.get_cvar("qlx_bdmMinRating"))
            max_rating = int(minqlxtended.get_cvar("qlx_bdmMaxRating"))
            print_change = bool(int(minqlxtended.get_cvar("qlx_bdmConsolePrintBdmChange")))
            for p_sid in self._player_stats:
                ...
                max_games_for_calc = int(minqlxtended.get_cvar("qlx_bdmNumGamesForCalculation"))
    ...
    def print_changes(self, n, steam_id, old, new):
        try:
            if n.value > 0:
                for x in range(n.value):
                    try:
                        pn = self.player(steam_id[x])
                        if pn:
                            name = pn.name
                        else:
                            name = self.db.lindex("minqlx:players:{}".format(steam_id[x]), 0)
                    except minqlxtended.NonexistentPlayerError:
                        name = self.db.lindex("minqlx:players:{}".format(steam_id[x]), 0)
                    minqlxtended.console_print("^3Player^7: {} ^3BDM Change^7: ^1Old^7=^6{}^7, ^4New^7=^2{}"
                                         .format(name, old[x], new[x]))
            elif n.value == -1:
                minqlxtended.console_print("^7--- ^6Only {} rounds played but need {} rounds,"
                                     " BDM is not being calculated ^7---"
                                     .format(self.rounds_played, int(minqlxtended.get_cvar("qlx_bdmMinRounds"))))
            elif n.value == -2:
                minqlxtended.console_print("^7--- ^6There were not enough players on each team, {} are needed. "
                                     "BDM stats not being calculated ^7---"
                                     .format(int(minqlxtended.get_cvar("qlx_bdmMinimumTeamSize"))))
        except Exception as e:
            if ENABLE_LOG:
    ...
    def set_initial_bdm(self, player, game_type):
        try:
            ...
            if response:
                ...
                m = float(minqlxtended.get_cvar("qlx_bdmMCalculation"))
                b = float(minqlxtended.get_cvar("qlx_bdmBCalculation"))
                if game_type in elo_dict[sid]:
                    bdm = int(m * int(elo_dict[sid][game_type]) + b)
                else:
                    bdm = int(minqlxtended.get_cvar("qlx_bdmDefaultBDM"))
                if bdm < int(minqlxtended.get_cvar("qlx_bdmMinRating")):
                    bdm = int(minqlxtended.get_cvar("qlx_bdmMinRating"))
                elif bdm > int(minqlxtended.get_cvar("qlx_bdmMaxRating")):
                    bdm = int(minqlxtended.get_cvar("qlx_bdmMaxRating"))
