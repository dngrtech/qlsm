...
class mydiscordbot(Plugin):
    ...
    * qlx_discord_extensions (default: "") discord extensions to load after initializing
    ...
    def __init__(self, discord_client=None):
        ...
        Plugin.set_cvar_once("qlx_discordMessagePrefix", "[ͳ⁷DISCORD]")
        ...
        Plugin.set_cvar_once("qlx_discord_extensions", "admin, status, topic_updater, qlstats")
        ...
        self.add_hook("chat", self.handle_ql_chat, priority=minqlxtended.Priority.LOWEST)
        self.add_hook(
            "player_connect", self.handle_player_connect, priority=minqlxtended.Priority.LOWEST
        )
        self.add_hook(
            "player_disconnect",
            self.handle_player_disconnect,
            priority=minqlxtended.Priority.LOWEST,
        )
        ...
        self.add_hook(
            "game_end", self.handle_game_countdown_or_end, priority=minqlxtended.Priority.LOWEST
        )
    ...
    def get_game_info(game):
        ...
        # Game.red_score / Game.blue_score do not exist on this runtime; scores come
        # off team_scores, indexed by Team.index (_game.py:190).
        scores = game.team_scores
        red_score = scores[minqlxtended.Team.RED.index]
        blue_score = scores[minqlxtended.Team.BLUE.index]
        if (
            game.roundlimit in [blue_score, red_score]
            or red_score < 0
            or blue_score < 0
        ):
            return f"Match ended: **{red_score}** - **{blue_score}**"
        if game.state == "in_progress":
            return f"Match in progress: **{red_score}** - **{blue_score}**"
    ...
    def handle_ql_chat(self, player, msg, channel, _recipient):
        """
        ...
        :param: _recipient: the direct-message recipient, when the chat is a private message; unused here
        """
        handled_channels = {
            "chat": "",
    ...
    def handle_player_connect(self, player, _is_bot):
        """
        ...
        :param: _is_bot: whether the player is a bot; unused here
        """
...
class SimpleAsyncDiscord(threading.Thread):
    ...
    def setup_extended_logger():
        ...
        file_path = os.path.join(
            minqlxtended.get_cvar("fs_homepath") or "", "minqlx_discord.log"
        )
        maxlogs = minqlxtended.Plugin.get_cvar("qlx_logs", int) or 5
        maxlogsize = minqlxtended.Plugin.get_cvar("qlx_logsSize", int) or 1024
    ...
    async def on_message(self, message):
        ...
        if message.channel.id in self.discord_relay_channel_ids:
            ...
            if len(content) > 0:
                minqlxtended.CHAT_CHANNEL.reply(
