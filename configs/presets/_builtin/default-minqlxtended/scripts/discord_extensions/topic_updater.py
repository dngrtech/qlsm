...
import minqlxtended
from minqlxtended import Plugin, NonexistentGameError
...
def get_game_info(game):
    ...
    # Game.red_score / Game.blue_score do not exist on this runtime; scores come off
    # team_scores, indexed by Team.index (_game.py:190).
    scores = game.team_scores
    red_score = scores[minqlxtended.Team.RED.index]
    blue_score = scores[minqlxtended.Team.BLUE.index]
    if game.roundlimit in [blue_score, red_score] or red_score < 0 or blue_score < 0:
    ...
    def update_topic_on_channels_and_keep_channel_suffix(self, channel_ids, topic):
        ...
        for channel_id in channel_ids:
            ...
            if channel_id not in self.discord_kept_topic_suffixes:
                minqlxtended.get_logger("mydiscordbot").debug(
