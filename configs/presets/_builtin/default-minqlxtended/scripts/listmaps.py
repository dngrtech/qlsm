...
class listmaps(minqlxtended.Plugin):
    def __init__(self):
        # Minqlx Hooks
            ...
            if line.startswith(b'VERSION'):
                ...
                # If routine check and it's not alright.
                    ...
                    try:
                        ...
                        minqlxtended.console_print("LISTMAPS Version Checking Error: {}".format(e))
                return
    ...
    def get_maps(self, player=None, msg=None, channel=None):
        ...
        with self.gather_maps():
            minqlxtended.console_command("fdir *.bsp")

        if player:
            player.tell("^4Server^7: The server maps have been stored in the file ^3{}^7.".format(listmaps.map_file))

        minqlxtended.console_print("The server maps have been stored in the file ^3{}".format(listmaps.map_file))
    ...
    def list_map_names(self, player, msg, channel):
        ...
        if "console" == channel:
            minqlxtended.console_print(title[0].strip("\n"))
            for line in maps:
                minqlxtended.console_print("^4" + line)
            minqlxtended.console_print(title[1].strip("\n"))
            return
