...
class protect(minqlxtended.Plugin):
    def __init__(self):
        ...
        self.add_hook("vote", self.process_vote, priority=minqlxtended.Priority.LOWEST)
            ...
            if line.startswith(b'VERSION'):
                ...
                # If routine check and it's not alright.
                    ...
                    try:
                        ...
                        minqlxtended.console_command("echo {}".format([e]))
                return
        ...
        try:
            ...
            # Kick Voting
                ...
                if player[0] in [-1, -2, -3]:
                    ...
                    return minqlxtended.Return.STOP_ALL
                if player[1] == minqlxtended.owner():
            ...
            minqlxtended.console_print("^1protect.py handle_vote_called Exception: {}".format([e]))
    ...
    def callvote_mute(self, caller, vote, player_name, player_id):
        try:
            ...
            minqlxtended.console_print("^1protect.py callvote_mute Exception: {}".format([e]))
    ...
    def callvote_to_spec(self, caller, vote, player_name, player_id):
        try:
            ...
            minqlxtended.console_print("^1protect.py callvote_to_spec Exception: {}".format([e]))
    ...
    def cmd_load_protects(self, player=None, msg=None, channel=None):
        try:
            ...
            try:
                ...
                m.write("{} ServerOwner\n".format(minqlxtended.owner()))
                m.close()
                temp_list = [int(minqlxtended.owner())]
                self.protect = temp_list
                if player:
                ...
                minqlxtended.console_print("^1Error ^3reading and/or creating the Protect list: {}".format([e]))
        except Exception as e:
            if player:
                player.tell("^1Error ^3reading the Protect list: {}".format([e]))
            minqlxtended.console_print("^1protect.py cmd_load_protects Exception: {}".format([e]))
        ...
        try:
            ...
            if action == "list":
                ...
                try:
                    ...
                    minqlxtended.console_print("^1Error ^3opening the Protect list file")
                    return
                except Exception as e:
                    player.tell("^1Error ^3reading the Protect list file: {}".format([e]))
                    minqlxtended.console_print("^1Error ^3reading the Protect list file: {}".format([e]))
                    return
                lines = f.readlines()
                f.close()
                for line in lines:
                ...
                try:
                    ...
                    if len(msg[2]) == 17:
                        ...
                        try:
                            ...
                            minqlxtended.console_print("^1protect exec_protect Find DB Name Exception: {}".format([e]))
                except ValueError:
            ...
            minqlxtended.console_print("^1protect.py cmd_protect Exception: {}".format([e]))
        ...
        if len(add_name) > 1:
            try:
                ...
                try:
                    ...
                    minqlxtended.console_print("^1Error ^3opening the Protect list file in update_line")
                    return minqlxtended.Return.STOP_EVENT
                except Exception as e:
                    player.tell("^1Error ^3reading the Protect list file in update_line: {}".format([e]))
                    minqlxtended.console_print("^1Error ^3reading the Protect list file in update_line: {}".format([e]))
                    return minqlxtended.Return.STOP_EVENT
                lines = f.readlines()
                f.close()
                ...
                try:
                    ...
                    minqlxtended.console_print("^1Error ^3opening the Protect list file for writing in update_line")
                    return minqlxtended.Return.STOP_EVENT
                except Exception as e:
                    player.tell("^1Error ^3reading the Protect list file for writing in update_line: {}"
                                .format([e]))
                    minqlxtended.console_print("^1Error ^3reading the Protect list file for writing in"
                                         " update_line: {}".format([e]))
                    return minqlxtended.Return.STOP_EVENT
            except Exception as e:
                minqlxtended.console_print("^1protect.py update_line Exception: {}".format([e]))
        return minqlxtended.Return.STOP_EVENT
    ...
    def cmd_setpass(self, player, msg, channel):
        if len(msg) < 2:
            ...
            return minqlxtended.Return.STOP_EVENT
        minqlxtended.console_command("set g_password \"{}\"".format(msg[1]))
    ...
    def cmd_unsetpass(self, player, msg, channel):
        minqlxtended.console_command("set g_password \"\"")
