...
class kills(minqlxtended.Plugin):
    def __init__(self):
    ...
    def handle_kill(self, victim, killer, mod):
        self.process_kill(victim, killer, mod)
        return

    @minqlxtended.thread
    def process_kill(self, victim, killer, mod):
        try:
            if self._supported_gametype:
                msg = None
                killer_steam_id = str(killer.steam_id)
                victim_steam_id = str(victim.steam_id)
                if killer_steam_id[0] in ["9", "0"] or victim_steam_id[0] in ["0", "9"]:
                    return
                # SPEED was a velocity magnitude minqlx's stats dict carried per kill; the
                # engine doesn't hand that over anymore, so compute it from live velocity.
                killer_velocity = killer.velocity
                killer_speed = (killer_velocity.x ** 2 + killer_velocity.y ** 2 + killer_velocity.z ** 2) ** 0.5
                if killer_speed > self.get_cvar("qlx_killsSpeedMinimum", int) and self._killMonitor[8]:
                    ...
                    if self.game.state == "in_progress":
                        ...
                        if self.db.exists(speed_value):
                            if int(killer_speed) > int(self.db[PLAYER_KEY.format(killer_steam_id) +
                                                                          ":highspeed"].split(".")[0]):
                                self.db[PLAYER_KEY.format(killer_steam_id) + ":highspeed"] =\
                                    int(killer_speed)
                        else:
                            self.db[PLAYER_KEY.format(killer_steam_id) + ":highspeed"] = int(killer_speed)
                        ...
                        msg = "^1SPEED ^3{}^1! ^7{} ^1{}^7:^1{}^7 {}".format(int(killer_speed), killer.name,
                                                                             killer_score, victim_score, victim.name)
                        self.add_killer(str(killer.name), "SPEED")
                    else:
                        msg = "^1SPEED ^3{}^1! ^7{}^7 :^7 {} ^7(^3warmup^7)".format(int(killer_speed),
                                                                                    killer.name, victim.name)

                if mod == "GAUNTLET" and (self._killMonitor[0] or self._killMonitor[1]):
                    # AIRBORNE/SUBMERGED were minqlx stats-dict booleans; minqlxtended carries
                    # neither, so read the same underlying engine state directly instead.
                    if self._is_airborne(victim) and self._is_airborne(killer) and not self._is_submerged(victim) and\
                            not self._is_submerged(killer) and self._killMonitor[1]:
                        if self._play_sounds and not self._playing_sound:
                            self._playing_sound = True
                    ...
                    if self._is_airborne(victim) and not self._is_submerged(victim):
                        if self._play_sounds and not self._playing_sound:
                            self._playing_sound = True
                    ...
                    if self._is_airborne(victim) and self._is_airborne(killer) and not self._is_submerged(victim) \
                            and not self._is_submerged(killer):
                        if self._play_sounds and not self._playing_sound:
                            self._playing_sound = True
                ...
                elif mod == "TELEFRAG" and self._killMonitor[6] and not self._is_teamkill(killer, victim):
                ...
                elif mod == "TELEFRAG" and self._killMonitor[7] and self._is_teamkill(killer, victim):
                    if self._play_sounds and (self._roundActive or self.game.state == "warmup") and\
                            not self._playing_sound:
                        self._playing_sound = True
            ...
            minqlxtended.console_print("^1kills process_kill Exception: {}".format([e]))

    # AIRBORNE/SUBMERGED/TEAMKILL were minqlx stats-dict booleans passed in with every kill.
    # minqlxtended's kill event carries neither, so these read the same underlying engine
    # state directly (ground_entity_num, waterlevel) or derive it from team membership.
    def _is_airborne(self, player):
        return player.entity.s.ground_entity_num == minqlxtended.MAX_GENTITIES - 1

    def _is_submerged(self, player):
        return player.entity.waterlevel > 0

    def _is_teamkill(self, killer, victim):
        # Team.FREE is excluded so FFA telefrags are never counted as team kills.
        return killer.team == victim.team and killer.team != minqlxtended.Team.FREE
    ...
    def handle_end_game(self, aborted):
    ...
    def exec_cmd_pummel(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_pummel Exception: {}".format([e]))
    ...
    def exec_cmd_airpummel(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_airpummel Exception: {}".format([e]))
    ...
    def exec_cmd_grenades(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_grenades Exception: {}".format([e]))
    ...
    def exec_cmd_rocket(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_rocket Exception: {}".format([e]))
    ...
    def exec_cmd_plasma(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_plasma Exception: {}".format([e]))
    ...
    def exec_cmd_airrail(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_airrail Exception: {}".format([e]))
    ...
    def exec_cmd_telefrag(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_telefrag Exception: {}".format([e]))
    ...
    def exec_cmd_teamtelefrag(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_teamtelefrag Exception: {}".format([e]))
    ...
    def exec_cmd_speedkill(self, player, msg):
        try:
            ...
            minqlxtended.console_print("^kills exec_cmd_speedkill Exception: {}".format([e]))
    ...
    def add_killer(self, killer, method):
        try:
            ...
            minqlxtended.console_print("^kills add_killer Exception: {}".format([e]))
    ...
    def sound_play(self, path):
        try:
            ...
            minqlxtended.console_print("^kills sound_play Exception: {}".format([e]))
        self._playing_sound = False
    ...
    def player_id(self, search, player):
        ...
        try:
            ...
            minqlxtended.console_print("^kills player_id Exception: {}".format([e]))
            return minqlxtended.Return.STOP_ALL
        if target_player == 0:
    ...
    def find_player(self, name):
        try:
            ...
            minqlxtended.console_print("^1specqueue find_player Exception: {}".format([e]))
    ...
    def cmd_kills_monitor(self, player=None, msg=None, channel=None):
        try:
            ...
            minqlxtended.console_print("^kills cmd_kills_monitor Exception: {}".format([e]))
