...
class irc(minqlxtended.Plugin):
    def __init__(self):
        self.add_hook("chat", self.handle_chat, priority=minqlxtended.Priority.LOWEST)
        self.add_hook("unload", self.handle_unload)
        self.add_hook("player_connect", self.handle_player_connect, priority=minqlxtended.Priority.LOWEST)
        self.add_hook("player_disconnect", self.handle_player_disconnect, priority=minqlxtended.Priority.LOWEST)
    ...
    def handle_msg(self, irc, user, channel, msg):
        ...
        if channel.lower() == self.relay.lower():
            if cmd in (".players", ".status", ".info", ".map", ".server"):
                ...
                minqlxtended.CHAT_CHANNEL.reply("[IRC] ^6{}^7:^2 {}".format(user[0], " ".join(msg)))
            ...
            if len(msg) > 1 and msg[0].lower() == ".auth" and self.password:
                ...
                def f():
                    try:
                        minqlxtended.COMMANDS.handle_input(IrcDummyPlayer(self.irc, user[0]), " ".join(msg[1:]), IrcChannel(self.irc, user[0]))
    ...
    def server_report(self, channel):
        ...
        if game.state == "in_progress":
            if game.type_short == "race" or game.type_short == "ffa":
                ...
                # Game.red_score / Game.blue_score do not exist on this runtime; scores
                # come off team_scores, indexed by Team.index (_game.py:190).
                scores = game.team_scores
                ginfo = "The score is \x02\x0304{}\x03 - \x0302{}\x03\x02".format(
                    scores[minqlxtended.Team.RED.index], scores[minqlxtended.Team.BLUE.index])
...
class IrcChannel(minqlxtended.AbstractChannel):
    name = "irc"
...
class IrcDummyPlayer(minqlxtended.AbstractDummyPlayer):
...
class SimpleAsyncIrc(threading.Thread):
    ...
    def run(self):
        ...
        logger = minqlxtended.get_logger("irc")
