...
class players_db(minqlxtended.Plugin):
    def __init__(self):
    ...
    def save_perms(self):
        ...
        try:
            ...
            minqlxtended.console_print("^1ERROR Opening perms file: {}".format(e))
            return minqlxtended.Return.STOP_ALL
        for player in playerlist:
            steam_id = player.split(":")[2]
            if len(str(steam_id)) == 17:
                h.write("{}:{}".format(steam_id, self.db.get(player)) + "\n")
        minqlxtended.console_print("^1Finished saving player permissions to {}".format(PERMS_FILE))
        h.close()
    ...
    def enter_perms(self):
        ...
        minqlxtended.console_print("^1Finished entering player permissions to the database.")
        h.close()
    ...
    def player_name(self, steam_id):
        try:
            ...
            minqlxtended.console_print("^1players_db player_name Exception {}: {}".format(steam_id, [e]))
            name = self.db.lindex(PLAYER_KEY.format(steam_id), 0)
        return name
    ...
    def show_sid_info(self, asker, msg):
        try:
            ...
            minqlxtended.console_print("^1players_db show_sid_info Exception: {}".format([e]))
            return
