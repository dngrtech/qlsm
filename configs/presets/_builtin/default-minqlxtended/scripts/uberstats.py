...
class uberstats(minqlxtended.Plugin):
  ...
  def handle_game_end(self, aborted):
    # game_end now passes just the aborted flag; the per-player stats this handler
    # reports come from handle_stats(), not from this event's payload.
    if not aborted and self.best_kpm:
