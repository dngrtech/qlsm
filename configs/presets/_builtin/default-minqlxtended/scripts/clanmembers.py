...
class clanmembers(minqlxtended.Plugin):
    def __init__(self):
        ...
        self.add_hook("userinfo", self.handle_userinfo, priority=minqlxtended.Priority.HIGH)
    ...
    def unload_overlapping_commands(self):
        try:
            clan = minqlxtended.Plugin._loaded_plugins['clan']
            remove_commands = set(['clan'])
            for cmd in clan.commands.copy():
                # cmd.name is now just the primary name (a str); cmd.names is the full
                # list this intersection check needs. set.intersection() on a bare str
                # would iterate its characters instead of matching, so it must be .names.
        ...
        for line in res.iter_lines():
            if line.startswith(b'VERSION'):
                ...
                # If routine check and it's not alright.
                    ...
                    except Exception as e: minqlxtended.console_command("echo {}".format(e))
                return
    ...
    def cmd_clan(self, player, msg, channel):
        ...
        if len(msg) < 2:
            if tag_key in self.db:
                del self.db[tag_key]
                cs = minqlxtended.parse_infostring(minqlxtended.configstring(index))
                del cs["cn"]
                del cs["xcn"]
    ...
    def cmd_loadClanTag(self, player=None, msg=None, channel=None):
        try:
            ...
            try:
                ...
                m.close()
                tempList = []
                tempList.append(int(minqlxtended.owner()))
                self.clanTagMembers = tempList
                if player:
