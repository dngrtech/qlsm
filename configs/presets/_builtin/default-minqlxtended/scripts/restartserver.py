...
class restartserver(minqlxtended.Plugin):
    def __init__(self):
        # cvars
    ...
    def handle_player_disconnect(self, player, reason):
        try:
            ...
            minqlxtended.console_print("^1restartserver handle_player_disconnect Exceptions: {}".format(e))
    ...
    def check_restart_time(self):
        try:
            ...
            try:
                ...
                minqlxtended.console_print("^1restartserver check_restart_time Timer Exception: {}".format(e))
            ...
            while self.checking_restart:
                if (restart_time[1] <= int(time.strftime("%j")) or restart_time[0] < year) and\
                    ...
                    minqlxtended.console_print("^1RestartServer^7: Restarting the empty server after the scheduled time of {}"
            ...
            minqlxtended.console_print("^1restartserver check_time Exceptions: {}".format(e))
        finally:
            self.checking_restart = False
    ...
    def remove_conflicting_time_commands(self):
        if self.get_cvar("qlx_restartUseTime", bool):
            loaded_scripts = minqlxtended.Plugin._loaded_plugins
            ...
            if len(msg) > 1:
                if msg[1].lower().startswith("h"):
                    ...
                    try:
                        ...
                        minqlxtended.console_print("^1restartserver restart_server Exceptions: {}".format(e))
            else:
                minqlxtended.console_print("^1Restart Server script is issuing a quit command because {}restart was issued"
