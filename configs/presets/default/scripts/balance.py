...
class balance(minqlx.Plugin):
    def __init__(self):
        ...
        self.balance_cmd_used_this_map = False
        ...
        self.set_cvar_once("qlx_balanceAdminOverrideLevel", "5")
    ...
    def cmd_balance(self, player, msg, channel):
        # ADDED: Once per map logic with admin override cvar
        ...
        required_override_level = self.get_cvar("qlx_balanceAdminOverrideLevel", int)
        
        # Owner also subject to this cvar for consistency with shuffle testing logic
        can_override = (player_permission_level is not None and player_permission_level >= required_override_level)

        if self.balance_cmd_used_this_map:
            if not can_override:
                player.tell("The !balance command has already been used on this map.")
                return minqlx.RET_STOP_ALL
            else:
                player.tell("Balance limit active, but overriding (level {} required).".format(required_override_level))
        ...
        # ADDED: Set flag after command logic initiated
        self.balance_cmd_used_this_map = True
