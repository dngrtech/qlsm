...
function InstanceDetailsModal({ instanceId, isOpen, onClose, onInstanceDeleted, onInstanceUpdated, onOpenEditConfig, onOpenHostDrawer, serverStatus }) {
  ...
  const handleToggleLanRate = async () => {
    if (!instanceId || !instance || !canToggleLanRate || lanRateForcedOn) return;
  ...
  // QLSM runs minqlxtended hosts at 99k: show it on and locked.
  const lanRateForcedOn = isLanRateForcedOn(hostShape);
  const lanRateDisplayEnabled = Boolean(instance?.lan_rate_enabled) || lanRateForcedOn;
  const lanRateUnsupportedReason = lanRateForcedOn || (!canToggleLanRate && !instance?.lan_rate_enabled)
                          ...
                          <div className="flex flex-col items-start gap-2">
                            <div className="flex items-center gap-2">
                              <button type="button" onClick={handleToggleLanRate}
                                disabled={lanRateUpdating || actionLoading || isBusyStatus || !canToggleLanRate || lanRateForcedOn}
