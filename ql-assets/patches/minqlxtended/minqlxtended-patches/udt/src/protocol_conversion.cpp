...
#include "string.hpp"
#include "utils.hpp"
// PATCH: upstream's normal build apparently
// gets atoi() visibility transitively (Windows/MSVC header pollution, or
// jumbo/unity-build ordering); it isn't pulled in by any header in this
// minimal vendored subset when each .cpp is compiled as its own TU on
// gcc/Linux. Added explicitly rather than silently failing to link.
#include <cstdlib>
