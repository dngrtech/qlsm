"""
serverchecker — minqlxtended plugin for live server status.
...
The key keeps its `minqlx:` prefix on purpose. It is an internal contract with
QLSM's live status poller (ui/task_logic/service_runtime.py), not a minqlx API,
and renaming it would only buy a runtime-aware reader on the other side.
"""
...
class serverchecker(minqlxtended.Plugin):
    ...
    def _register_hooks(self):
        """Hook every event this plugin watches.

        minqlxtended validates a handler's signature at registration rather
        than at dispatch (_plugin.py:241), so one stale signature stops the
        whole plugin from loading. Kept separate from __init__ so the
        signatures can be verified without starting the update thread.
        """
    ...
    def on_game_start(self):
        # game_start dispatches no arguments on this runtime (_events.py:698).
        self._match_start_time = time.time()
        self._update_status_async()

    def on_game_end(self, aborted):
        # game_end passes `aborted` instead of minqlx's stats dict
        # (_events.py:711). Either way the match is over.
        self._match_start_time = None
        self._update_status_async()
